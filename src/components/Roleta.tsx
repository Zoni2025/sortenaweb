...
export default function Roleta({ items, onResult, size = 400, disabled = false, targetIndex, eligibleIndices = [] }: RoletaProps) {
  ...
  // Retorna o índice do item sob o ponteiro (topo = -π/2)
  ...
  const spin = () => {
    ...
    // ===== 2. CALCULAR ROTAÇÃO ALVO =====
    // Para que o CENTRO da fatia resultIndex fique no ponteiro (-π/2):
    // rot + resultIndex * sliceAngle + sliceAngle/2 = -π/2
    // rot = -π/2 - resultIndex * sliceAngle - sliceAngle/2
    const targetRot = -Math.PI / 2 - resultIndex * sliceAngle - sliceAngle / 2

    // ===== 3. CALCULAR ROTAÇÃO FINAL (com voltas extras) =====
    // IMPORTANTE: usar número INTEIRO de voltas para que finalRot
    // seja visualmente idêntico a targetRot (diferem por múltiplo exato de 2π)
    const extraSpins = 6 + Math.floor(Math.random() * 4) // 6, 7, 8 ou 9 voltas inteiras
    ...
    // Adicionar voltas extras (múltiplo inteiro de 2π)
    finalRot -= extraSpins * 2 * Math.PI
    ...
    const animate = (now: number) => {
      ...
      if (progress < 1) {
        ...
        // ===== 5. ANIMAÇÃO TERMINOU =====
        // current = startRot + totalDelta = finalRot
        // finalRot é visualmente idêntico a targetRot (diferem por N * 2π inteiro)
        // Então o ponteiro está EXATAMENTE sobre a fatia resultIndex

        setIsSpinning(false)
        if (audioCtxRef.current) playWinSound(audioCtxRef.current)

        // Reportar o item que está visualmente sob o ponteiro
        const visualIndex = getSliceAtPointer(current)
